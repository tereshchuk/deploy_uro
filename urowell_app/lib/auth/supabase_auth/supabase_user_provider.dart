...
class UrowellAppSupabaseUser extends BaseAuthUser {
  UrowellAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> urowellAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = UrowellAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
